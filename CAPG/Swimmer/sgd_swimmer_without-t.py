...
import numpy as np
for env_id in [HalfCheetahEnv, HumanoidEnv, SimpleHumanoidEnv]:
    for batchsize in [5000]:
        for learning_rate in [0.05,0.02]:
            for i in range(3):
                seed = np.random.randint(1,10000)
                env_name = "SGD"+str(env_id)
                hidden_sizes = (32, 32)
                ...
                algo = CAPG(
                    ...
                    max_path_length=500,
                    discount=0.99,
                    log_dir='./sgd_without-t/' + env_name + "seed" + str(seed) + '/',
                )
                algo.train()
