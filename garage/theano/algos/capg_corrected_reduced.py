...
class CAPG(object):
    ...
    @staticmethod
    def grad_norm(s_g):
        return np.linalg.norm(s_g)
    @staticmethod
    def normalize_gradient(s_g):
        l2_norm = np.linalg.norm(s_g)
        return s_g/l2_norm
    @staticmethod
    ...
    def train(self):
        ...
        while(self.batch_size < self.n_timestep - j):
            ...
            for _ in range(self.n_sub_itr):
                ...
                # first component: summation of advantage[max(i,j)] * g_llh_i * dot(g_llh, d_vector)
                # compute the list of loglikelihood gradient
                gradient_loglikelihood = []
                for step in range(len(sub_paths[0]["rewards"])):
                    ll_t = self.f_mix_lh(sub_observations[0][step].reshape(1,-1), sub_actions[0][step].reshape(1,-1))
                    ll_t = self.flatten_parameters(ll_t)
                    gradient_loglikelihood.append(ll_t)
                gradient_loglikelihood = np.array(gradient_loglikelihood)
                # compute H scalars
                inner_product = np.dot(gradient_loglikelihood, d_vector)
                advantages = sub_advantages[0]
                fst = 0
                #print(gradient_loglikelihood.shape)
                #print(advantages.shape)
                for idg, g in enumerate(gradient_loglikelihood):
                    for idp, p in enumerate(inner_product):
                        a_ij = g * p * advantages[max(idg, idp)]
                        fst += a_ij
                '''
                ...
                while(n_sub < self.minibatch_size):
                    ...
                    gradient_loglikelihood = []
                    for step in range(len(sub_paths[0]["rewards"])):
                        ll_t = self.f_mix_lh(sub_observations[0][step].reshape(1, -1),
                                             sub_actions[0][step].reshape(1, -1))
                        ll_t = self.flatten_parameters(ll_t)
                        gradient_loglikelihood.append(ll_t)
                    gradient_loglikelihood = np.array(gradient_loglikelihood)
                    # compute H scalars
                ...
                #fst = [x/n_sub for x in fst]
                #hv = [x/n_sub for x in hv]
                # gradient as sum
                hv = self.flatten_parameters(hv)
                fst = fst/len(sub_paths_all)
                hv = hv/len(sub_paths_all)
                ...
                g_d = fst + hv + self.gradient_backup
    ...
    def outer_optimize(self, itr, samples_data):
        ...
        s_g = self.flatten_parameters(s_g)
